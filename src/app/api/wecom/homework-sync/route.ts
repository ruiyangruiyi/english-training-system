...
// 作业识别关键�?const HOMEWORK_KEYWORDS = ['作业', 'homework', '今日任务', '练习', '背诵', '完成'];
// 作业格式标记
const HOMEWORK_MARKERS = ['【作业�?, '[作业]', '📝'];

// POST /api/wecom/homework-sync - 接收群消息，识别并同步作�?export async function POST(request: NextRequest) {
  try {
...
function checkIsHomework(content: string): boolean {
  ...
  // 检查格式标�?  for (const marker of HOMEWORK_MARKERS) {
...
function parseHomework(content: string): {
  ...
  // 提取标题（第一行或�?0字符�?  let title = content.split('\n')[0];
...
function parseDueDate(content: string): Date {
  ...
  // 匹配"X月X�?
  const dateMatch = content.match(/(\d{1,2})�?\d{1,2})[日号]/);
  if (dateMatch) {
  ...
  // 匹配"周X"
  const weekMatch = content.match(/�?[一二三四五六日天])/);
  if (weekMatch) {
